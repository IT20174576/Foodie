                            ...
                            <tbody>
                                <?php
                                    ...
                                    while($myrow = mysqli_fetch_assoc($myresult)){
                                        ...
                                        $itemsql = "SELECT * FROM `product` WHERE Id = $pId";
                                        $itemresult = mysqli_query($conn, $itemsql);
                                        $itemrow = mysqli_fetch_assoc($itemresult);
                                        $pName = $itemrow['Name'];
                                        $pPrice = $itemrow['Price'];
                                        $pDesc = $itemrow['Desc'];
                                        $pCategorieId = $itemrow['CategorieId'];

                                        echo '<tr>
                                                <th scope="row">
                                                    <div class="p-2">
                                                    <img src="/Foodie/img/product-'.$pId. '.jpg" alt="" width="70" class="img-fluid rounded shadow-sm">
